...
\begin{equation}
\begin{split}
Var(X_{1} +X_{2}-2X_{3}) = Var(Var(X_{1}) + Var(X_{2}) + 2 \times Cov(X_{1},X_{2}))\\ + 2^2 \times Var(X_{3}) \\ - 2 \times 2 \times Cov(Var(X_{1}) + Var(X_{2}) + 2 \times Cov(X_{1},X_{2}),X_{3})
\end{split}
\end{equation}
...
\end{equation}
\end{enumerate}
\item Consider the random vector \textbf{X$'$} = $\left[X_{1},X_{2},X_{3},X_{4}\right]$ with mean vector $\mu'= \left[1,2,3,4\right]$ and covariance matrix \[\Sigma=\]
\end{enumerate}
\end{document}
